...
function getTrustedOrigins() {
  ...
  if (vercelUrl) {
    origins.add(vercelUrl.startsWith('http') ? new URL(vercelUrl).origin : `https://${vercelUrl}`);
  }
...
export function rejectUnexpectedSearchParams(request: Request) {
...
export function rejectUntrustedOrigin(request: Request) {
  ...
  if (referer) {
    try {
      ...
      if (!trustedOrigins.has(refererOrigin) && refererOrigin !== requestOrigin) {
        return NextResponse.json({ error: 'Untrusted request origin.' }, { status: 403 });
      }
    } catch {
      return NextResponse.json({ error: 'Untrusted request origin.' }, { status: 403 });
