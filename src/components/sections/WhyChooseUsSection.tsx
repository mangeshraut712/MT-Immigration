...
const reasons = [
  {
    ...
    description:
      "The same attorney owns the case strategy from consultation through the next filing decision.",
  },
  {
    icon: Clock,
    title: "Deadline-Focused Work",
    description:
      "Urgent interviews, court dates, and government response deadlines are treated with priority.",
  },
  {
    icon: BadgeCheck,
    title: "Clear Scope and Fees",
    description:
      "Consultations, filing work, and next steps are framed in plain language before work begins.",
  },
  {
    icon: Shield,
    title: "Disciplined Intake",
    description:
      "The site now validates submissions, keeps the AI key server-side, and rate-limits public forms.",
  },
];
            ...
            >
              {reasons.map((reason, index) => (
                <motion.div
                  ...
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: shouldReduceMotion ? 0 : index * 0.08, duration: shouldReduceMotion ? 0 : 0.3 }}
                  className="group flex gap-4 md:gap-5 rounded-xl md:rounded-2xl border border-white/5 bg-zinc-900/50 p-5 md:p-6 backdrop-blur-sm transition-colors hover:border-white/10 hover:bg-zinc-900"
                >
            ...
            <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {practiceFocus.map((area, index) => (
                <motion.div
                  ...
                  className="flex h-full flex-col justify-between rounded-xl md:rounded-2xl border border-white/5 bg-zinc-900/50 p-6 md:p-8 backdrop-blur-sm transition-all duration-300 hover:border-white/10 hover:bg-zinc-900"
                >
